...
contact page uses, styled to match. It spans the three tracks beside the
...
- Signature collections on Dock Styles uses the same `.config-grid` /
  `.config-card` pattern as the two anchoring systems on the Anchoring page:
  a 16/10 photo, an `.h-card` title, and one paragraph ending in a
  `.text-link`. Two cards side by side so the reader can compare them. New
  two-option runs should reuse this pattern rather than inventing a card.
- Callouts are one component, not a set of one-offs. Any panel whose job is to
  ...
  lifted `--surface` fill, a `--line` hairline, a 4px olive rail down the
  leading edge and `--shadow-card`. The shared rule lives under "Callouts" in
  `style.css`; `.callout` is the generic class, and `.info-card`,
  `.warranty-panel` and `.placeholder-box` inherit from the same selector group
